...
export default async function ProyectoDetallePage({ params }: { params: { id: string } }) {
    ...
    // Telegram URL con detalles pre-cargados del proyecto
    const telegramMessage = encodeURIComponent(
        `¡Hola C7Dev! Vengo de tu portafolio y me interesa cotizar un proyecto basado en:\n\n` +
        `📌 Proyecto: ${proyecto.titulo}\n` +
        `🏷️ Categoría: ${proyecto.categoria || 'Web'}\n` +
        `🛠️ Tecnologías: ${(proyecto.tags || []).join(', ') || 'N/A'}\n\n` +
        `¿Podríamos hablar sobre los detalles, costo y tiempo de entrega?`
    );
    const telegramCotizarUrl = `https://t.me/+573244259132?text=${telegramMessage}`;
                        ...
                        <div className="flex flex-col gap-4">
                            {proyecto.demo_url && (
                                <a
                                    ...
                                    className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-neon-gold to-amber-600 text-black font-bold uppercase tracking-wider rounded-xl hover:shadow-[0_0_30px_rgba(255,215,0,0.4)] transition-all group"
                                >
                            ...
                            <a
                                href={telegramCotizarUrl}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-gradient-to-r from-[#0088cc] to-[#005580] text-white font-bold uppercase tracking-wider rounded-xl hover:shadow-[0_0_30px_rgba(0,136,204,0.4)] transition-all group"
                            >
                                <Send className="w-5 h-5" />
                                Cotizar este Proyecto en Telegram
                                <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                            </a>
                        </div>
