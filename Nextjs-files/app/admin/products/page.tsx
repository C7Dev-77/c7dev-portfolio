...
export default function ProductsManager() {
    ...
    const handleEdit = (product: any) => {
        ...
        setFormData({
            ...
            precio: product.price_cents ? product.price_cents / 100 : 0, // Convertir centavos → USD
            ...
            categoria: product.category || 'Código',
            destacado: product.is_featured || false,
            activo: product.is_active !== false,
            orden: product.display_order || 0
        });
        setShowForm(true);
    };
    ...
    const handleToggleDestacado = async (id: string, currentDestacado: boolean) => {
        const { error } = await (supabase.from('products') as any)
            .update({ is_featured: !currentDestacado })
            .eq('id', id);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Convertir precio USD → centavos para la tabla products
        const priceCents = Math.round(formData.precio * 100);

        // Generar slug a partir del nombre si es nuevo
        ...
        const payload: any = {
            ...
            display_order: formData.orden,
            has_free_version: Boolean(formData.link_free),
        };
        ...
        if (editingId) {
            ...
            // slug y storage_path son requeridos en INSERT
            payload.slug = slug;
            payload.storage_path = formData.link_paid || formData.link_free || slug;
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex gap-1.5 flex-wrap">
                                        ...
                                        {/* Mostrar links configurados */}
                                        {product.link_free && (
                                            <span className="text-[10px] bg-green-500/10 text-green-400 px-2 py-0.5 rounded">✓ Link Gratis</span>
                                        )}
                                        {product.link_paid && (
                                            <span className="text-[10px] bg-neon-gold/10 text-neon-gold px-2 py-0.5 rounded">✓ Link Pago</span>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="hidden md:flex items-center gap-2">
                                    <button
                                        onClick={() => handleToggleDestacado(product.id, product.is_featured)}
